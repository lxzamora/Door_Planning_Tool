...
select cldr_dt as date, orig_sic_cd as orig_sic,  fnl_dest_sic_cd as dest_sic, orig_in_wgt as weight_in, orig_in_vol as cube_in, orig_out_wgt as weight_out, orig_out_vol as cube_out from FLO_FLOW_PLAN_SUMMARY_VW where orig_shft_cd = 'OTB' and date >= '2014-05-16' and date <= '2014-05-22' 
and orig_sic='UGV' distribute on (orig_sic, dest_sic); 
create temp table current_loc_load_plan as
select * from tbl_loc_load_plan_rds_vw where REPL_LST_UPDT_TMST>'2014-05-22' and current_sic='UGV' and shift_cd='O';
create temp table ffo_extracts_otb as (select a.date as date, a.orig_sic as orig_sic, 'O' as orig_shift,a.dest_sic as dest_sic, '' as load_to_mode1, '' as load_to_shift1, case b.day_frt_ind when 'D' then b.frst_fac_sic||' D'  else b.frst_fac_sic end as load_to_sic1, '' as load_to_shift2, b.scnd_fac_sic as load_to_sic2, '' as load_to_shift3,b.thrd_fac_sic as load_to_sic3, b.must_clear_sic as must_clear_sic, '' as must_clear_shift, case b.day_frt_ind when 'D' then 'Y'  else '' end as daylane_freight, a.weight_in as weight_in, a.cube_in as cube_in, a.weight_out as weight_out, a.cube_out as cube_out 
...
where A.launch_date = '2013-10-21' and A.orig_sic='NGV' and B.door_sic is null order by A.door_sic; 
select A.door_sic from (select distinct door_sic from 
sic_doors where launch_date= '2013-10-14' and orig_sic='NGV') as A
left join
(select distinct door_sic from 
sic_doors where launch_date='2013-10-21' and orig_sic = 'NGV') as B on A.door_sic = B.door_sic where B.door_sic is null;