--the following tables are all in prd_whseview, change the date before you run!
create temp table ffo_extracts_temp as
select cldr_dt as date, orig_sic_cd as orig_sic,  fnl_dest_sic_cd as dest_sic, orig_in_wgt as weight_in, orig_in_vol as cube_in, orig_out_wgt as weight_out, orig_out_vol as cube_out from FLO_FLOW_PLAN_SUMMARY_VW where orig_shft_cd = 'FAC' and date >= '2015-09-18' and date <= '2015-09-24' 
and orig_sic='XNW' distribute on (orig_sic, dest_sic); 
create temp table current_loc_load_plan as
select * from tbl_loc_load_plan_rds_vw where REPL_LST_UPDT_TMST>'2015-09-24' and current_sic='XNW' and shift_cd='N';
create temp table ffo_extracts_otb as (select a.date as date, a.orig_sic as orig_sic, 'F' as orig_shift,a.dest_sic as dest_sic, '' as load_to_mode1, '' as load_to_shift1, case b.day_frt_ind when 'D' then b.frst_fac_sic||' D'  else b.frst_fac_sic end as load_to_sic1, '' as load_to_shift2, b.scnd_fac_sic as load_to_sic2, '' as load_to_shift3,b.thrd_fac_sic as load_to_sic3, b.must_clear_sic as must_clear_sic, '' as must_clear_shift, case b.day_frt_ind when 'D' then 'Y'  else '' end as daylane_freight, a.weight_in as weight_in, a.cube_in as cube_in, a.weight_out as weight_out, a.cube_out as cube_out 
...
select orig_sic,
       ...
       case when sum(mark_bypass)/5.0 >= 1.0 then 'X' else '' end as bypass,
